...
h_train = @(P) func_M5_PRF_sc_vxl(P,Ts_10,HR,RF,ind_BOLD_10,GS,1);
% PRF_par = ga(h_train,length(ub),[],[],[],[],lb,ub,[],[],ga_opts);
PRF_par = fmincon(h_train,PRF_par,[],[],[],[],lb,ub,[],options);

h = @(P) func_M5_PRF_sc_vxl(P,Ts_10,HR,RF,ind_BOLD_10,GS,0);
[obj_function,CRF_sc,RRF_sc,HR_conv,RF_conv,r_PRF_sc,yPred, HR_Fconv_MR, HR_Sconv_MR, RF_Fconv_MR, RF_Sconv_MR] = h(PRF_par);
...
xPhys = [HR_Fconv_MR, HR_Sconv_MR, RF_Fconv_MR, RF_Sconv_MR];  xPhys = detrend(xPhys,'linear');
